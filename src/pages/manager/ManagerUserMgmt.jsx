...
import {
  ...
  StatusBadge, SearchBar, TabNav, Avatar, InputField, ConfirmModal, Modal, Skeleton,
  PermissionGate,
} from '../../components'
import { fetchUsers, inviteOwner } from '../../services/api'
import './ManagerUserMgmt.css'

export default function ManagerUserMgmt() {
  ...
  const emptyStateLabel = {
    pending: 'No Pending Verification Found',
    verified: 'No Verified Users Found',
    flagged: 'No Flagged Users Found',
    all: 'No Users Found',
  }[statusTab] || 'No Users Found'
  ...
  return (
    <PageShell
      ...
      bottomNav={<BottomNav role="manager" activeTab={activeTab} onTabChange={handleTabChange} />}
    >
      <div className="aum stagger-children">
        <div className="aum__toolbar">
          <div className="aum__toolbar-search">
            <SearchBar placeholder="Search users by name, email, or ID..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <PrimaryButton icon="person_add" fullWidth={false} onClick={() => { setInviteError(''); setInviteOpen(true) }}>
            New User
          </PrimaryButton>
        </div>
        ...
        <div className="aum__list">
          {loading ? (
            ...
            filtered.map((u) => {
              const isPending = isPendingVerification(u)
              const isVerified = u.status === 'verified'
              const isFlagged = u.status === 'flagged' || u.status === 'rejected'
              return (
                <GlassCard key={u.id} className="aum__card animate-slide-up">
                  <div className="aum__card-row">
                    <Avatar initials={u?.initials || ''} size="md" verified={isVerified} />
                    ...
                    {isVerified && (
                      <SecondaryButton className="aum__inline-action" fullWidth={false} onClick={() => navigate(`/kyc-review/${u.id}`)}>
                        View
                      </SecondaryButton>
                    )}
                    {isFlagged && (
                      <PermissionGate code="kyc.update">
                        <SecondaryButton className="aum__inline-action" fullWidth={false} onClick={() => navigate(`/kyc-review/${u.id}`)}>
                          Resubmit
                        </SecondaryButton>
                      </PermissionGate>
      ...
      <Modal
        open={inviteOpen}
        onClose={() => { if (!inviteLoading) { setInviteOpen(false); setInviteError('') } }}
        title="Create NRI Owner"
        size="sm"
      >
        <div style={{ display: 'grid', gap: 'var(--space-3)' }}>
          ...
          />
          {inviteError && (
            <p style={{ color: 'var(--status-danger)', fontSize: 'var(--fs-caption)' }}>{inviteError}</p>
          )}
          <PrimaryButton
            icon="person_add"
