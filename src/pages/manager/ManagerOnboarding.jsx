...
import './ManagerOnboarding.css'

// V2 onboarding states (08_database_schema.md §4 — onboarding_workflow_state_enum).
const STEPS = [
  { key: 'visit_requested', label: 'Visit Requested', icon: 'mail', tsField: 'visit_requested_at' },
  { key: 'visit_scheduled', label: 'Visit Scheduled', icon: 'event', tsField: 'visit_scheduled_at' },
                ...
                {isOpen && (
                  <div className="aonb__body">
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 'var(--space-2)' }}>
                      <button
                        type="button"
                        onClick={() => navigate(`/manager-onboarding/${wf.id}`)}
                        style={{
                          background: 'var(--primary)', color: 'var(--text-on-primary)',
                          border: 'none', borderRadius: 'var(--radius-md)',
                          padding: '8px 14px', fontWeight: 'var(--fw-semibold)',
