...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    fetchAdminDashboard()
      ...
      .catch((err) => reportError(err, { context: 'AdminDashboard.fetchAdminDashboard', silent: true }))
  }, [])
  ...
  return (
    <PageShell
      ...
      bottomNav={
        <BottomNav role="manager" activeTab={activeTab} onTabChange={onTabChange} />
      }
    >
      <div className="adm stagger-children">
        ...
        {/* ── Visit Requests (negotiation inbox + onboarding workflows) ── */}
        <GlassCard interactive className="adm__highlight-card animate-slide-up" onClick={() => navigate('/manager-visits')}>
          <div className="adm__highlight-row">
            <div className="adm__highlight-icon" style={{ background: 'rgba(19, 200, 236, 0.14)' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '22px', color: 'var(--primary)' }}>event_available</span>
            </div>
            <div className="adm__highlight-info">
              <p className="adm__highlight-title">Visit Requests</p>
              <p className="adm__highlight-sub">Review incoming requests for your assigned properties — accept, counter-propose, or reject.</p>
            </div>
        ...
        <div className="adm__action-grid">
          ...
          <GlassCard interactive className="adm__action-card" onClick={() => navigate('/manager-onboarding')}>
